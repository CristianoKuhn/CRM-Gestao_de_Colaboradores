...
import { Colaborador, FormularioInstancia, FormularioTemplate, RespostaCampo, WorkflowDefinicao } from '../types';
...
//
// Substitui, para o fluxo de Avaliação de Experiência, a lógica que antes
// só marcava `avaliacoesCompletas` sem formulário nenhum. A Avaliação 180°
// continua no ModalAvaliacao180 até o Sprint 4 (migração planejada).
//
// Nota de cálculo: `resultado` (média/parecer) fica vazio ao concluir aqui —
// o motor de cálculo completo (calculoEngine) é entregue no Sprint 4. Isso
// não impede a conclusão: "atrasada" nunca bloqueia a execução, e a mesma
// regra vale para o cálculo — a instância pode ser concluída antes do
// parecer existir, que é preenchido/retrocompletado quando o motor chegar.
// ═══════════════════════════════════════════════════════════════════
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      ...
      try {
        ...
        const templateAtivo = templates.find((t) => t.ativo) || templates[templates.length - 1];
        if (!templateAtivo) {
        ...
        const workflowDoTemplate = workflows.find((w) => w.id === templateAtivo.workflowId) || null;

        let instanciaAtual = instanciasExistentes.find(
          (i) => i.estadoWorkflow !== 'concluida' && i.estadoWorkflow !== 'arquivada'
        );
        if (!instanciaAtual) {
          instanciaAtual = {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-bold text-slate-800">Avaliação de Experiência</h2>
